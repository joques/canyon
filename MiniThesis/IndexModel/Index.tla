...
VARIABLES   store,          \* A data store mapping keys to values.
            object,         \* The set of open snapshot objects.
            snapshotStore,  \* Snapshots of the store for each objects.
...
TypeInvariant ==    \* The type invariant.
    ...
    /\ snapshotStore \in [ObjId -> Store]
    /\ written \in [ObjId -> SUBSET Key]
    /\ missed \in [ObjId -> SUBSET Key]
    
objectLifecycle ==
    /\ \A t \in object :    \* If store != snapshot & we haven't written it, we must have missed a write.
...
Spec == \* Initialize state with Init and transition with Next.
    Init /\ [][Next]_<<store, object, snapshotStore, written, missed>>
----------------------------------------------------------------------------
THEOREM Spec => [](TypeInvariant /\ objectLifecycle)
=============================================================================
\* Modification History
\* Last modified Fri Oct 28 11:32:13 CAST 2022 by goodwill
